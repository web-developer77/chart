...
class CurveComparison extends CIQ.UI.ModalTag {
	...
	renderLegend() {
		...
		for (let curveId in termStructure.curves) {
			...
			let { color, symbol, Date: dateObj, error, live } = curve;
			...
			if (curve.relativeDate) {
				...
				if (multiplier === 0) {
					const mainCurve = termStructure.curves._main_curve;
	...
	selectItem(obj) {
		...
		const { stx } = this.context;
		const { termStructure } = stx;
		const { _main_curve } = termStructure.curves;
