 ...
 * @param {function} [cb] A callback function to execute once the symbol change is complete. The
 * 		first argument of the callback function is a reference to the chart engine.
 *
 ...
 */
/**
 * Sets the [lookup driver]{@link CIQ.ChartEngine.Driver.Lookup} to be used with the
 * [cq-lookup web component]{@link WebComponents.cq-lookup}.
 *
 ...
 * `UIContext.UISymbolLookup` field.
 *
...
CIQ.UI.KeystrokeHub = function (node, context, params) {
	...
	this.tabIndex = -1;
	// Get the value of 10% of the total context element height. Used to normalize y coordinates when sorting elements by position
	this.yNormalizationThreshold =
		this.context.topNode.getBoundingClientRect().height / 10;
...
CIQ.UI.KeystrokeHub.prototype.handler = function (obj) {
	...
	if (
		...
		!(
			this.context.isModal() &&
			!(e.key === "Escape" || e.key === "Esc" || e.key === "Tab")
		)
	) {
...
class Chart {
		...
		if (restore) {
			...
			CIQ.ChartEngine.restoreLayout(
				...
				function () {
					...
					if (onChartReady) onChartReady(stx);
				},
				chartId
			);
		} else {
	...
	notifySizeChanges(uiContext) {
		...
		if (breakpoint !== stx.chart.breakpoint) {
			channelWrite(
				channels.breakpoint || "channel.breakpoint",
				breakpoint,
				stx
			);
			stx.notifyBreakpoint(breakpoint);
		}
		channelWrite(
			channels.containerSize || "channel.containerSize",
			{ width, height },
	...
	/**
	 * Maps numeric widths to responsive design breakpoints. Breakpoints include small (break-sm),
	 * medium (break-md), and large (break-lg).
	 *
	 * @param {number} width The width in pixels for which a breakpoint is determined.
	 * @return {string} The breakpoint: "break-sm", "break-md", or "break-lg".
	 *
	 * @alias getBreakpoint
	 * @memberof CIQ.UI.Chart.prototype
	 * @since 7.5.0
