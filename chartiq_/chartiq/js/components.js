...
class InfoToggleDropdown extends CIQ.UI.ContextTag {
	...
	setContext(context) {
		...
		this.options = [...this.optionsMenu.querySelectorAll("cq-item")]; // ciao to NodeList
		...
		this.options.forEach((option) => {
			...
			option.selectFC = function () {
				selectItem({ target: this[0] }); // capture focused element from keyStroke in scroll.js
			};
		});
	 ...
	 * text is "Info on" when the drop-down toggle is inactive; "Info off", when the drop-down
	 * toggle is active.
	 *
	...
	setTooltip(value) {
		...
		if (
			...
			const nextState = !this.toggleEl.currentValue ? "on" : "off";
			tooltip.innerText = stx.translateIf(`Info ${nextState}`);
		}
	}
	...
	applyValues(channels) {
		const crosshair = this.channelRead(channels.crosshair);
		const headsUp = this.channelRead(channels.headsUp) || {};
		const { dynamic } = headsUp;
		const staticHu = headsUp.static; // reserved keyword

		const staticHudContainer = this.context.topNode.querySelector(
			"cq-hu-static"
		);
		if (staticHudContainer) {
			staticHudContainer.style.display =
				CIQ.isMobile && headsUp.floating ? "none" : "";
		}

		// timeout safeguards against issues caused by multiple listeners writing to channels where the
		// order callbacks are executed in may result in the behavior of this method being overwritten
		setTimeout(() => {
			if (dynamic && CIQ.isMobile) {
				this.channelWrite(
					channels.headsUp,
			...
			if (crosshair && !staticHu && CIQ.isMobile && !headsUp.floating) {
				this.channelWrite(
					channels.headsUp,
					Object.assign({}, headsUp, { static: true })
				);
			}

			if (!crosshair && (staticHu || headsUp.termstructure)) {
...
if (!CIQ.ChartEngine.Driver) {
	 ...
	 *
	 * **Note:** A {@link CIQ.ChartEngine.Driver.Lookup} must be provided. If none is provided,
	 * the default is used which displays no results.
	 *
	 * Use `[CIQ.UI.Context.setLookupDriver](CIQ.UI.Context.html#setLookupDriver)` to link the
	 * driver to this web component.
	 *
	 * Set `<cq-lookup cq-uppercase>` to force free-form text to be converted to uppercase.
	 *
	 * Set `<cq-lookup cq-exchanges=_comma-delimited-exchange-list_>` to specify an override list
	 * of financial instrument exchanges for use by the lookup driver.
	 *
	 * To turn off the result window, modify the CSS to
	 * `.stxMenuActive cq-lookup cq-menu { opacity: 0 }`.
	 *
	 * If you wish to preload default results on initial load, rather than an empty result pane,
	 * use the following code to be placed at the very end of the `startUI()` function.
	 *
	 * ```
	 * let defaultResults = [
	 *     {
	...
	class Lookup extends CIQ.UI.ContextTag {
		 ...
		 *
		 * @param {function} cb The function to be set as a callback; for example, an
		 * 		implementation of {@link CIQ.UI.Context#changeSymbol}.
		 *
		 * @alias setCallback
		 * @memberof WebComponents.cq-lookup#
		 */
		...
		/**
		 * Sets a {@link CIQ.ChartEngine.Driver.Lookup}. If none is set, the context lookup driver
		 * is used (see {@link CIQ.UI.Chart#initLookup}). If a driver is not available, the input
		 * box is active but does not present a drop-down list.
		 *
		 * @param {CIQ.ChartEngine.Driver.Lookup} driver The lookup driver to be set.
		 *
		 * @alias setDriver
		 * @memberof WebComponents.cq-lookup#
		 */
...
class Menu extends HTMLElement {
	...
	hide() {
		...
		if (dropdown) dropdown.disablekeyboardNavigation();
	}
...
class MessageToaster extends CIQ.UI.ContextTag {
	 ...
	 *
	 * @param {object} notification Data from a "notification" event. See
	 * 		[notificationEventListener]{@link CIQ.ChartEngine~notificationEventListener}.
	 ...
	 * concurrent toasts.
	 *
	 * This function is the "notification" event listener.
	 *
	 * @param {object} notification Data from a "notification" event. See
	 * 		[notificationEventListener]{@link CIQ.ChartEngine~notificationEventListener}.
	 * @param {string} notification.message Text to display in the toast notification. Strings
...
class Scroll extends CIQ.UI.BaseComponent {
	...
	keyStroke(hub, key, e) {
		...
		var items = this.querySelectorAll("cq-item:not(.item-hidden)");
		...
		node.find("cq-item").removeAttr("cq-focused");
		...
		// If the selected item is at either end it might not be visible.
		if (CIQ.trulyVisible(items[i])) {
	...
	highlightFocusedItem() {
		...
		const focused = this.querySelector("cq-item[cq-focused]");
		if (focused) {
			...
			if (CIQ.Studies) {
				for (var id in stx.layout.studies) {
					...
					if (edit && !sd.editFunction) {
						...
						if (!edit) edit = newChild.find("cq-label")[0];
						CIQ.UI.stxtap(edit, editStudy(this, id));
					}
				}
			}
...
if (!Scroll) {
	...
	class MenuDropDown extends Scroll.classDefinition {
		...
		connectedCallback() {
			...
			// Look for keyboard navigable items in the dropdown
			this.childItems = this.querySelectorAll(":scope > cq-scroll");
			this.focusedChildIndex = -1;
			// For now, it's hard setting to the one cq-scroll element that appears.
			// In the future, this will be set via up/down arrow in this components
			// own keyStroke method.
			if (this.childItems.length > 0) this.focusedChildIndex = 0;
		}
		...
		disablekeyboardNavigation() {
			if (this.keyboardNavigation) {
				...
				this.childItems[this.focusedChildIndex] &&
				this.childItems[this.focusedChildIndex].keyboardNavigation
			) {
				// Reach into the active element to get its reference to the keystroke hub
				this.childItems[
					this.focusedChildIndex
				].keyboardNavigation.setKeyControlElement();
			}
		}

		keyStroke(hub, key, e) {
			if (!this.keyboardNavigation) return;
			// If the menu contains a focused keyboardNavigable element. Pass keyboard control down
			if (this.focusedChildIndex >= 0) {
				this.keyboardNavigation.setKeyControlElement(
					this.childItems[this.focusedChildIndex]
				);
			} else {
				super.keyStroke(hub, key, e);
			}
		}
	}
